...
use crate::chain::{Board, CellData, IterId, Detector, board_to_cols, apply_gravity};
...
impl BruteforceSearch {
    ...
    fn dfs_deduplicated(
        ...
        // 空欄を試す
        Self::dfs_deduplicated(base_board, candidates, idx + 1, placed, results, baseline, max_results, trial_count, first_chain_top, seen_hashes);
        if results.len() >= max_results {
            return;
        }
